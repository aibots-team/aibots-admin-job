...
build-win: # Build project for Windows | 构建Windows下的可执行文件
	env CGO_ENABLED=0 GOOS=windows go build -trimpath -ldflags "$(LDFLAGS)" -o $(PROJECT).exe $(PROJECT).go
	@echo "Build project successfully"

.PHONY: build-mac
build-mac: # Build project for MacOS | 构建MacOS下的可执行文件
	env CGO_ENABLED=0 GOOS=darwin go build -trimpath -ldflags "$(LDFLAGS)" -o $(PROJECT) $(PROJECT).go
	@echo "Build project successfully"

.PHONY: build-linux
build-linux: # Build project for Linux | 构建Linux下的可执行文件
	env CGO_ENABLED=0 GOOS=linux go build -trimpath -ldflags "$(LDFLAGS)" -o $(PROJECT).exe $(PROJECT).go
	@echo "Build project successfully"
